...
A screen-hidden assistant for Windows. **Visible on your monitor, invisible to screen-sharing tools** (Zoom, Teams, Meet, OBS). It streams everything — answers, live speech-to-text, and screenshot analysis — through the app's own backend over WebSocket, and can **listen continuously** to spoken questions and answer them hands-free.

> Pipeline: **microphone → backend speech-to-text → backend (Cerebras) → answer on screen**
...
- Speech is captured with a voice-activity detector and streamed live (raw PCM16) to the backend's speech-to-text bridge over WebSocket as you talk.
- Background noise/filler phrases ("um", "thank you", "okay", …) are filtered out before being treated as a question.
- The recognized question is sent to the backend automatically (routed to **Cerebras**) and the answer streams in.

---

## 🖼️ Screenshot analysis

Press **Ctrl+Shift+S** to capture the screen and send it to the backend for vision analysis (**Cerebras** gemma-4-31b) — useful for coding questions or slides shared during an interview.
...
│   └── lib/                     ← interviewSocket.js, sttSocket.js, voice.js, screenAnalyze.js, prompts.js, …
├── package.json
└── README.md
```
...
- No API keys needed in the app — the backend holds its own provider keys server-side.
